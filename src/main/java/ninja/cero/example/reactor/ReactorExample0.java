...
public class ReactorExample0 {
    public static void main(String[] args) {
        new ReactorExample0().run();
    }

    void run() {
        Mono<Map<Student, Flux<Score>>> result = fetchStudents("sg")
                .log("after-fetchStudents")
                .collectMap(
                        student -> student,
                        student -> fetchScores(2017, student.id)
                                .log("after-fetchScores")
                );

        result.subscribe(map -> map.forEach(((student, scoreFlux) -> {
            scoreFlux.log("print")
